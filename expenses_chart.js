...
let expenseChart = new Chart(ctx, {
    ...
    options: {}//fill in things later.

})  //had to reset these a few times for it to look readable.

// TODO (optional) replace with colors of your choice. The array can have as many or as few colors as you like 
let chartColors = [ 'darkred', 'orange', 'dodgerblue', 'mediumseagreen', 'slateblue', 'violet' ]
//pick as many as you want or differnet colors.
...
function addExpenseToChart(name, amount) {  
    ...
    /*modular %  division remainer.this is a math wiz issues for me.
    meaning sometime the math necassary isn't understood it's needed */
    console.log(colorCount,color)
    expenseChart.data.datasets[0].backgroundColor.push(color)
    //ask to update with new data added.
    expenseChart.update()
}
...
window.addEventListener('keyup', function(){
    if (event.keyCode === 13){  // do not understand this strike through on event 
        let inputElements = [expenseNameInput,expenseAmountInput,addExpenseButton]
        if( inputElements.includes(document.activeElement)){
            addExpenseButton.click()//preforms the click with enter press no mouse action needed
            expenseNameInput.focus()//must seselct the correct variable here.
        }
    }
})
